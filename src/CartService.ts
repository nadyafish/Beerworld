...
interface CartService {
    ...
    clearItemfromCart(item: CartItem, currentCart: CartItem[]): Promise<void>;
}
...
class ServerCartService implements CartService {
    ...
    async clearItemfromCart(item: CartItem, currentCart: CartItem[]):Promise<void> {
        let existingItem = currentCart.find(value => value.id === item.id);
        ...
        let url: string = hasItem ? `${DATA_URL}/cart/${item.id}` : `${DATA_URL}/cart`;
    ...
    async clearCart():Promise<void> {
        ...
        currentCart.map((item: CartItem) => {
            this.clearItemfromCart(item, currentCart);
        })
        return;
    }
